# messages.py

messages = {
    "welcome_message": {
        "image_url": "https://telegra.ph/file/ad7079ce8110af0f35771.png",
        "text": "✨ Привет!👻 Я - телеграмм бот Like me, твой персональный помощник в поиске новых знакомств и, возможно, даже любви! Я помогу тебе найти единомышленников, общаться и встречаться с интересными людьми. Давай начнем искать тебе идеального собеседника? \n \nЕсли да, то ответь пожалуйста на "
                "<b>два формальных вопроса.<\b>",
        "button_text": "Регистрация"
    },
    "age_request_message": {
        "text": "Перед тем как начать, давай создадим твой профиль. "
                "Напиши, сколько тебе лет 🎂 \n \n<b>*Напиши возраст в цифрах.<\b> "

    },
    "age_input_message": {
        "error_text_age_under_16": "Извини, но ботом могут пользоваться только лица старше <b>16 лет.<\b>",
        "error_text_invalid_data_type": "Пожалуйста, введите ваш возраст цифрами.",
        "text_consent_messages": "🧐 Для работы с ботом требуется твоё <b>разрешение<\b> на использование ссылки "
                                 "на твой Telegram профиль. Можно?",
        "button_text_yes": "Да 👍",
        "button_text_no": "Нет 👎"
    },
    "consent_yes_message": {
        "text": "Хорошо, с формальностями закончили, давай продолжим создавать твой профиль 😃\nКак тебя зовут?"
    },
    "consent_no_message": {
        "text": "Нам жаль, если передумаешь, будем ждать тебя 😔"
    },
    "ask_gender_message": {
        "text": "Выбери пол",
        "button_text_male": "Мужчина 🧢",
        "button_text_female": "Женщина 👒"
    },
    "ask_city_message": {
        "text": "В каком городе ты живешь? \n \n*Напиши <b>существующий город<\b>, иначе поиск будет некорректным. "
    },
    "ask_descriptions_message": {
        "text": "Расскажи немного о себе 📝 \n \n <b>*НЕ используй стикеры!<\b>. Иначе заполнение анкеты будет некорректным."
    },
    "ask_status_message": {
        "text": "Какую цель общения ты ищешь?",
        "button_text_status_1": "👋 Найти друзей",
        "button_text_status_2": "💕 Найти вторую половинку",
        "button_text_status_3": "💬 Просто пообщаться"
    },
    "ask_photo_message": {
        "text": "Теперь загрузи своё фото 🖼"
    },
    "photo_and_final_register_message": {
        "image_url": "https://telegra.ph/file/ad7079ce8110af0f35771.png",
        "text": "🗒 Твой профиль создан! Теперь ты можешь начать пользоваться ботом",
        "button_text": "Супер, давай начнем!"
    },
    "profile_edit_message": {
        "image_url": "https://telegra.ph/file/ad7079ce8110af0f35771.png",
        "text": "Этот функционал ещё не разработан. Приходи сюда позже :)",
        "button_text_back": "Назад"
    },
    "profile_delete_confirm_message": {
        "image_url": "https://telegra.ph/file/ad7079ce8110af0f35771.png",
        "text": "Ты точно хочешь удалить свой профиль? Это действие отменить нельзя",
        "button_text_confirm_delete": "✅ Да, удалить",
        "button_text_cancel": "❌ Нет, вернуться назад"
    },
    "profile_deleted_message": {
        "image_url": "https://telegra.ph/file/ad7079ce8110af0f35771.png",
        "text": "Твой профиль удален. \nНам жаль, что ты уходишь. Если передумаешь, мы будем рады тебя видеть."
    },
    "main_screen_message": {
        "image_url": "https://telegra.ph/file/ad7079ce8110af0f35771.png",
        "text": "Добро пожаловать на главный экран, тут ты сможешь начать поиск знакомств, узнать о проекте и "
                "настроить свой профиль.",
        "button_text_start_searching": "🔍 Начать поиск 🔍",
        "button_text_profile": "🪪 Мой профиль",
        "button_text_about": "ℹ️ О Проекте"
    },
    "about_project_message": {
        "image_url": "https://telegra.ph/file/ad7079ce8110af0f35771.png",
        "text": " <b>'Like me'<\b> - это телеграмм бот полезных знакомств, разработанный с большой любовью для пользователей. "
                             "\nЗдесь ты можешь найти новых друзей, партнеров или просто пообщаться с интересными людьми. "
                             "\n<b>Преимущества 'Like me''<\b> включают в себя удобный и понятный интерфейс, возможность выбрать цель знакомства. <b>Главное преимущество<\b> проекта - он <b>полностью бесплатный<\b>, так что присоединяйся и наслаждайся общением! "
                                                        "\n \nС любовью, команда 'Like me' 💙️",
        "button_text_back": "Назад"
    }
}
